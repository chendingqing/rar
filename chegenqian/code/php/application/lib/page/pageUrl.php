...
class pageUrl
{
    ...
    public function show() {
        ...
        $this->url = Url::build(Request::instance()->root(),$this->parameter);
        /* 计算分页信息 */
        ...
        $pageStart = "<div class='mx-page-box'><ul class='mx_page'>";
        $pageEnd = "</ul></div>";
        //上一页
        $up_row  = $this->nowPage - 1;
        //$up_page = $up_row > 0 ? '<li><a class="prev" href="' . $this->url($up_row) . '">' . $this->config['prev'] . '</a></li>' : '';
        $up_page = $up_row > 0 ? '<li class="prev"><a href="' . $this->url($up_row) . '"> <span class="hidden-480">' . $this->config['prev'] . '</span></a></li>' : ' <li><a href="javascript:;" class="mx-page-disabled" data-page="0">上一页</a></li>';
        //下一页
        $down_row  = $this->nowPage + 1;
        //$down_page = ($down_row <= $this->totalPages) ? '<li><a class="next" href="' . $this->url($down_row) . '">' . $this->config['next'] . '</a></li>' : '';
        $down_page = ($down_row <= $this->totalPages) ? '<li class="next"><a href="' . $this->url($down_row) . '"><span class="hidden-480">' . $this->config['next'] . '</span> </a></li>' : '<li><a href="javascript:;" class="mx-page-disabled" data-page="0">下一页</a></li>';
        //第一页
        $the_first = '';
        ...
        //替换分页内容
        //dump($this->config['theme']);
        // dump($this->config['header']);
        $page_str = str_replace(
            array('%HEADER%', '%NOW_PAGE%', '%UP_PAGE%', '%DOWN_PAGE%', '%FIRST%', '%LINK_PAGE%', '%END%', '%TOTAL_ROW%', '%TOTAL_PAGE%'),
            array($this->config['header'], $this->nowPage, $up_page, $down_page, $the_first, $link_page, $the_end, $this->totalRows, $this->totalPages),
            $this->config['theme']);
        $page_str = $pageStart.$page_str.$pageEnd;
        return "{$page_str}";
    }
}